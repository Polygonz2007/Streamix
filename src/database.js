...
export function create_artist(name) {
    // Add
    const query = db.query("INSERT INTO artists (name) VALUES (?)");
...
export function get_artist_name(name) {
    ...
    const query = db.query("SELECT id FROM artists WHERE artists.name = ?");
...
export function create_album(name, artist_id, album_image, year) {
    ...
    // Add
    const query = db.query("INSERT INTO albums (name, artist_id, img, year) VALUES (?, ?, ?, ?)");
...
export function get_album_name(name) {
    // Search for artist by name
    const query = db.query("SELECT id FROM albums WHERE albums.name = ?");
    const result = query.get(name);
    if (!result)
        return false;
...
export function get_album_image(id) {
    const query = db.query("SELECT img FROM albums WHERE albums.id = ?");
...
export function create_track_artist(track_id, artist_id) {
    const query = db.query("INSERT INTO track_artists (track_id, artist_id) VALUES (?, ?)");
...
export function get_all_track_meta() {
    ...
    const query = db.query(`SELECT id FROM tracks`);
...
export function get_track_path(id) {
    const query = db.query("SELECT path FROM tracks WHERE tracks.id = ?");
    const result = query.get(id);
    if (!result)
        return false;
...
export function get_track_by_path(path) {
    const query = db.query("SELECT id FROM tracks WHERE tracks.path = ?");
...
export function create_track_frame(track_id, format, index, data) {
    const query = db.query("INSERT INTO track_frames (track_id, format, frame_index, frame_data) VALUES (?, ?, ?, ?)");
...
export function create_search_entry(track_id, track, album, artists) {
    ...
    const query = db.query("INSERT INTO search_index (track_id, track, album, artist) VALUES (?, ?, ?, ?)");
...
export function search(string, max_results) {
    ...
    if (search_string) { // Check there is anything
        // Search!
        const query = db.query(`SELECT track, album, artist, track_id, bm25(search_index, 10.0, 5.0, 2.0) AS rank
                                FROM search_index
        ...
        const query = db.query(`SELECT id as track_id FROM tracks LIMIT ?`);
...
export function log_error(type, file, description) {
    ...
    // Do the thang
    const query = db.query(`INSERT INTO errors (type, file, description) VALUES (?, ?, ?)`);
